...
\begin{itemize}
\item \textit{density-based}: il raggruppamento avviene analizzando l'intorno di ogni punto dello spazio. In particolare, viene considerata la densitï¿½ di punti in un intorno di raggio fissato.
...
L'altra applicazione sviluppata, chiamata "GPS Alarm", invece considera una circonferenza di raggio 20metri, centrata in un monumento che ne delimita la sua area "protetta". Se il visitatore non si trova all'interno di quest'area la probabilit\`{a} di osservare il monumento corrispondente ad essa \`e pari a zero, altrimenti ï¿½ 1. Questo solo per poter conisderare solo i reperti che \`e ragionevole pensare un turista possa voler inquadrare. Naturalmente questi approcci non permettono di classificare un monumento da una distanza maggiore di quella considerata, anzi produrrï¿½ una classificazione errata, in quanto verranno considerate solo i monumenti in cui le coordinate del visitatore si trovano all'interno delle loro aree protette.\\
...
MobileNet \`e un'architettura rilasciata da Google e resa disponibile open source, contenente una famiglia di modelli di computer vision per dispositivi mobili per TensorFlow. \`E stata progettata per massimizzare efficacemente l'accuratezza, tenendo presente le risorse limitate per un'applicazione integrata su dispositivo. Per questo motivo \`e stata scelta per adattarsi alla nostra esigenza di classificare immagini tramite un'applicazione installata su un dispositivo mobile. La particolarit\`{a} di questa architettura \`e l'uso di convoluzioni separabili per costruire reti neurali profonde leggere. L'intuizione di questi sviluppatori \`e stata di introdurre due semplici iperparametri che riescono ad equilibrare efficacemente latenza e accuratezza, permettendo all'architettura di adattarsi a diverse applicazioni. La latenza ,in informatica, di un sistema \`e l'intervallo di tempo che intercorre fra il momento in cui arriva l'input e il momento in cui \`e disponibile il suo output. In altre parole: la latenza non \`e altro che una misura della velocitï¿½ di risposta di un sistema.
Ma che cosa si intende per convoluzioni separabili? Sono delle convoluzioni formate da due convoluzioni: una convoluzione in profonditï¿½ e una convoluzione con un filtro di dimensione $1\times 1$ chiamata convoluzione puntuale.
La convoluzione in profonditï¿½ applica un singolo filtro a ciascun canale di ingresso e all'output generato viene applicato il filtro della convoluzione puntuale. Riassumendo: la convoluzione standard consente di filtrare e combinare gli input in un nuovo set di output in un unico passaggio, mentre la convoluzione separabile lo fa in due passaggi, ognuno svolto da uno strato di convoluzione diverso.
\begin{figure}[h!]
\centering
...
Ora questi filtri devono essere combinati per creare nuove funzionalit\`{a}. Pertanto, per generare queste nuove caratteristiche ï¿½ necessario lo strato con la convoluzione puntuale che calcoli una combinazione lineare dell'uscita della convoluzione in profondit\`{a}, mediante la convoluzione con filtro di dimensione $1 \times 1$ di profondit\`{a} M e applicato agli N canali di output.\\
...
Per iniziare qualsiasi attivit\`{a} di incremento, \`e necessario innanzitutto inizializzare un oggetto Pipeline, che punta a una cartella (\textit{directory})  in cui ï¿½ archiviato il set di dati dell'immagine originale:
\begin{lstlisting}[language={Python},tabsize=2,basicstyle=\footnotesize]
import Augmentor
p=Augmentor.Pipeline("/percorso/dove/risiedono/le/immagini")
\end{lstlisting}
La variabile p ora contiene un oggetto di tipo Pipeline ed ï¿½ stata inizializzata con l'elenco di immagini trovate nella cartella di origine.\\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=35mm]{originale.eps}
\qquad\qquad
\includegraphics[width=45mm]{ruotata.eps}
\caption{Esempio di foto dell'Arco di Settimio Severo ruotata a destra.}
\end{figure}

Esistono molte altre operazioni che si possono applicare alle immagini, come ad esempio: zoom, conversione in bianco e nero, ritaglio, distorsione, riflessione ecc... Infine Augmentor permette di eseguire un controllo per in dividuare eventuali doppioni generati e li cancella.
...
\begin{lstlisting}[language={Python},tabsize=2,basicstyle=\footnotesize]
base_model=MobileNet(input_shape=(224,224,3),include_top=False,input_tensor=Input(shape=(224,224,3)),pooling=None)
...
\end{list}
Per questa configurazione sono stati settati il moltiplicatore di larghezza $\alpha ,\rho =1$, rispettivamente il moltiplicatore di larghezza e di risoluzione. Non compaiono tra gli argomenti perch\`e, se omessi, sono settati gi\`{a} ad 1 di default. Discorso analogo per il valore relativo al dropout, che  di default vale $0.001$.\\
...
out = GlobalAvgPool2D(name='avg_pools26')(out)#se c'ï¿½ pooling lo levo
out = Dropout(0.4)(out)
y_pred=Dense(n_classes,activation='softmax',kernel_initializer='random_uniform',kernel_regularizer=l2(.0005),name='last_layers26')(out)
\end{lstlisting}
...
model.fit(X_train,Y_train_cat,batch_size=100,epochs=10000,verbose=2,callbacks=[lr_sheduler,csv_logger, checkpointer],validation_data=(X_val,Y_val_cat),shuffle=True)
\end{lstlisting}
Con il metodo fit si allena il modello, inserendo tra gli argomenti tutti i parametri necessari per settarlo, anche il metodo di minimizzazione da usare durante la fase di back-propagation, in questo caso SGD (discesa del gradiente).\\
...
%riportare accuratezza dei modelli provati con diversi batch size
...
Un'Activity \`e un'interfaccia utente. Ogni volta che si usa un'App, generalmente, si interagisce con una o pi\`{u} schermate, mediante le quali si consultano dati o si immettono input. Essa \`e il punto di partenza di ogni applicazione ed \`e la componente con cui l'utente ha il contatto pi\`{u} diretto.\\
Un Service svolge un lavoro, generalemente lungo e continuato, che viene svolto interamente in \textit{background} senza bisogno di interazione diretta con l'utente. I Service hanno un'importanza basilare nella programmazione proprio perch\'{e}, spesso, preparano i dati che le activity devono mostrare all'utente, permettendo una reattivit\`{a} maggiore nel momento della visualizzazione.\\
...
Un Broadcast Receiver \`e un componente che reagisce ad un invio di messaggi a livello di sistema, con cui Android notifica l'avvenimento di un determinato evento, ad esempio l'arrivo di un SMS o di una chiamata. Questi componenti sono utili per la gestione istantanea di determinate circostanze speciali.\\
Un Intent \`e un oggetto che associa due componenti separate, come due attivit\`{a}, in seguito dell'invocazione di una per l'altra. Esso rappresenta infatti l'intento di fare qualcosa, senza che la componente chiamante cessi la sua esistenza.
Un Fragment \`e strettamente legato alla propria activity, dalla quale riceve eventi in input, che possono essere elaborati mentre l'attivit\`{a} \`e in esecuzione. La particolarit\`{a} di questo componente risiede nel fatto che, un Fragment puï¿½ avere un'interfaccia utente totalmente sua, sempre per\`{o} all'interno di quella dell'activity. Si pu\`{o} pensare ad esso come un frammento dell'Activity, indipendente da essa, un esempio \`e dato da i menu a tendina al lato della schermata di un'applicazione.\\
...
Quando si inizia un progetto Android, la prima cosa da fare \`e istanziare l'activity principale, senza la quale non pu\`{o} essere costruita un'applicazione. La mainActivty, dal punto di vista del codice,  \`e una classe che estende la super classe Activity e dalla quale eredita dei metodi che verranno implementati tramite \textit{override}. In generale ci\`{o} che collegherï¿½ i layer con la classe sar\`{a} il documento, in formato .xml, chiamato: AndoridManifest. Esso raccoglie le informazioni necessarie al sistema per far girare qualsiasi porzione di codice all'interno dell'applicazione. Tra le altre cose il Manifest si occupa delle seguenti cose:
\begin{itemize}
...
In GPS, l'etichetta predetta ha una probabilit\`{a} maggiore di "match" con l'etichetta effettiva, in confronto ad Archlite, quando ci si trova in prossimit\`{a} del sito. Viceversa la probabilit\`{a} \`e pi\`{u} bassa quando la foto risulta "panoramica", ovvero quando il sito di interesse \`e lontano dall'obiettivo della fotocamera. Si \`e osservato che nei casi in cui si  \`e vicini al sito, se la predizione \`e corretta, la probabilit\`{a} registrata \`e sempre molto alta, mentre quando la previsione \`e errata, l'app GPS registra una probabilit\`{a} pi\`{u} bassa. Denotando un comportamento coerente con la "filosofia" con cui \`e stato costruito.\\
Si \`e riscontrato un fenomeno di overfitting: l'etichetta corrispondente alla  "Domus Augustana" \`e spesso tra i risultati pi\`{u} probabili, se non il pi\`{u} probabile. Proprio a causa di questo comportamento, i risultati sono stati inficiati dalla predominanza di questa etichetta. Nei casi in cui il valore predetto non corrisponde a quello effettivo, la Domus Augustana compare come risultato pi\`{u} probabile nel $50\%$ dei casi.\\
