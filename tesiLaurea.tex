...
\begin{figure}[t]
\centering
\includegraphics[scale=0.5]{schermata}
...
\section{MobileNet}
MobileNet \`e un'architettura rilasciata da Google e resa disponibile open source, contenente una famiglia di modelli di computer vision per dispositivi mobili per TensorFlow. \`E stata progettata per massimizzare efficacemente l'accuratezza, tenendo presente le risorse limitate per un'applicazione integrata su dispositivo. Per questo motivo \`e stata scelta per adattarsi alla nostra esigenza di classificare immagini tramite un'applicazione installata su un dispositivo mobile. L'architettura di mobileNet





Le MobileNet si basano su un'architettura semplificata che utilizza convoluzioni separabili in profondità per costruire reti neurali profonde leggere. Introduciamo due semplici hyperparameters globali che si scambiano efficacemente tra latenza e accuratezza. Questi parametri iper consentono al costruttore del modello di scegliere il modello di dimensioni giuste per la propria applicazione in base ai vincoli del problema.Questo documento propone una classe di architetture di rete che consente a uno sviluppatore di modelli di scegliere specificamente una piccola rete che soddisfa le restrizioni di risorse (latenza, dimensioni) per la loro applicazione. MobileNets si concentra principalmente sull'ottimizzazione della latenza, ma offre anche piccole reti. Le MobileNet sono costruite principalmente da convoluzioni separabili in profondità per ridurre il calcolo nei primi strati. Un approccio diverso per ottenere reti di piccole dimensioni è il restringimento, il factorizing o la compressione di reti preallenate. La compressione basata sulla quantizzazione del prodotto, l'hashing e la potatura, la quantizzazione vettoriale e la codifica di Huffman sono state proposte in letteratura. Inoltre sono state proposte varie fattorizzazioni per accelerare le reti pre-esistenti. Un altro metodo per la formazione di piccole reti è la distillazione che utilizza una rete più grande per insegnare una rete più piccola.

Architettura MobileNet:
Il modello di MobileNet si basa su convoluzioni separabili in profondità che è una forma di convoluzioni fattoriali che fanno scattare una convoluzione standard in una convoluzione in profondità e una convoluzione 1X1 chiamata convoluzione puntuale. Per MobileNets la convoluzione in profondità applica un singolo filtro a ciascun canale di ingresso. La convoluzione puntuale applica quindi una convoluzione 1X1 per combinare le uscite della convoluzione in profondità. Una convoluzione standard consente di filtrare e combinare gli input in un nuovo set di output in un unico passaggio. La convoluzione separabile in profondità la divide in due strati, uno strato separato per il filtraggio e uno strato separato per la combinazione. Questa fattorizzazione ha l'effetto di ridurre drasticamente il calcolo e la dimensione del modello. La Figura 2 mostra come una convoluzione standard 2 (a) è fattorizzata in una convoluzione in profondità 2 (b) e una 1X1 convoluzione in senso puntuale 2 (c).
Uno strato convoluzionale standard prende come input una mappa di caratteristiche di DF X DF XM F e produce una mappa di caratteristiche di DF XDF XN G dove DF è la larghezza e l'altezza spaziale di una caratteristica di input quadrata map1, M è il numero di canali di input (profondità di input) , DG è la larghezza e l'altezza spaziale di una mappa caratteristica di uscita quadrata e N è il numero di output
canale (profondità di uscita). Lo strato convoluzionale standard è parametrizzato da
kernel di convoluzione K di dimensione DK X DK X M X N dove DK è la dimensione spaziale del kernel che si presume essere quadrata e M è il numero di canali di input e N è il numero di canali di uscita come definito in precedenza. La mappa delle caratteristiche di output per la convoluzione standard assumendo il passo uno e il padding viene calcolato come: (1)pag2
Le convoluzioni standard hanno il costo computazionale di:
DK x DK x M x N x DF x DF
dove il costo computazionale dipende in modo moltiplicativo dal numero di canali di ingresso M, dal numero di canali di uscita N dalla dimensione del kernel Dk x Dk e dalla dimensione della mappa delle caratteristiche DF x DF. I modelli MobileNet riguardano ciascuno di questi termini e le loro interazioni. Innanzitutto utilizza convoluzioni separabili in profondità per interrompere l'interazione tra il numero di canali di uscita e la dimensione del kernel.
L'operazione di convoluzione standard ha l'effetto di filtrare le caratteristiche in base ai kernel convoluzionali e combinare le caratteristiche per produrre una nuova rappresentazione. Le fasi di filtraggio e combinazione possono essere suddivise in due fasi mediante l'uso di convoluzioni fattoriali denominate convoluzioni separabili in profondità per una sostanziale riduzione dei costi computazionali.La convoluzione separabile in profondità è composta da due strati: convoluzioni in profondità e convoluzioni puntuali. Usiamo le convoluzioni in profondità per applicare un filtro singolo per ciascun canale di ingresso (profondità di input). La convoluzione puntuale, una semplice convoluzione 1 x 1, viene quindi utilizzata per creare una combinazione lineare dell'output dello strato in profondità. MobileNets utilizza nonlinearità sia batchnorm che ReLU per entrambi i livelli.
La convoluzione in profondità con un filtro per canale di ingresso (profondità di input) può essere scritta come: (3) pag3, dove \hat{K} è il kernel della convoluzione i profondità di dimensione D_k\times D_k \times M, dove l'm-esimo filtro in \hat{K} è applicato al m-esimo canale in F per produrre m-esimo canale delle caratteristiche dell'output filtrato \hat{G}. La convoluzione in profondità ha un costo computazionale di: D_k\cdot D_k\cdot M\cdot D_F\cdot D_F.
La convoluzione in profondità è estremamente efficiente rispetto alla convoluzione standard. Tuttavia, filtra solo i canali di input, non li combina per creare nuove funzionalità. Pertanto, per generare queste nuove caratteristiche è necessario un ulteriore strato che calcoli una combinazione lineare dell'uscita della convoluzione in profondità mediante la convoluzione 1 x 1. La combinazione di convoluzione in profondità e convoluzione 1 x 1 (puntuale) è chiamata convoluzione separabile in profondità, che haun costo di:
 D_k\cdot D_k\cdot M\cdot D_F\cdot D_F + M\cdot N \cdot D_F \cdot D_F che è la somma delle due convoluzioni. Esprimendo la convoluzione come un processo in due fasi di filtraggio e combinazione otteniamo una riduzione nel calcolo di:
\frac{D_k\cdot D_k\cdot M\cdot D_F\cdot D_F + M\cdot N \cdot D_F \cdot D_F}{D_k\cdot D_k\cdot M\cdot N \cdot D_F \cdot D_F}=\frac{1}{N}+\frac{1}{D_K^2}
MobileNet utilizza 3 x 3 convoluzioni separabili in profondità che utilizzano da 8 a 9 volte meno calcoli rispetto alle convoluzioni standard con una piccola riduzione della precisione. La struttura MobileNet è costruita su convoluzioni separabili in profondità come menzionato nella sezione precedente, ad eccezione del primo strato che è a piena convoluzione. Definendo la rete in termini così semplici, siamo in grado di esplorare facilmente le topologie di rete per trovare una buona rete. L'architettura MobileNet è definita nella Tabella 1. Tutti gli strati sono seguiti da una non linearità di tipo batchnorm e ReLU con l'eccezione del livello finale completamente connesso che non ha non linearità e alimenta uno strato di softmax per la classificazione. La Figura 3 mette a confronto uno strato con convoluzioni regolari, non standard e ReLU non linearità allo strato fattorizzato con convoluzione in profondità, 1 x 1 convoluzione puntuale così come batchnorm e ReLU dopo ogni strato convoluzionale. Il campionamento verso il basso viene gestito con la continua convoluzione nelle convoluzioni in profondità e nel primo strato. Un pool medio finale riduce la risoluzione spaziale a 1 prima del livello completamente connesso. Conteggio delle convoluzioni in profondità e in senso orario come strati separati, MobileNet ha 28 strati.
